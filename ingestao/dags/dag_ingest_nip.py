...
DBT_SELECT = (
    "stg_nip api_bronze_nip api_prata_nip fat_reclamacao_operadora "
...
with DAG(
    ...
    materializar_nip_tipado = BashOperator(
        task_id="materializar_nip_tipado",
        cwd="/workspace",
        bash_command=(
    ...
    registrar_job = BashOperator(
        ...
        bash_command=registrar_job_command(
            "dag_ingest_nip",
            "nip",
            "ingestao_trimestral_nip",
        ),
    )

    extrair_nip >> carregar_nip >> materializar_nip_tipado >> staging_nip >> registrar_job
