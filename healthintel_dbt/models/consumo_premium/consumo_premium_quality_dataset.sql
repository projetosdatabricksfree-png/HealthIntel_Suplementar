...
totais as (
    select
        ...
        sum(case when documento_quality_status = 'VALIDO' then total_registro else 0 end) as total_valido
    from agregado
    group by 1
)
...
select
    ...
    round(totais.total_valido::numeric / nullif(totais.total_fonte, 0), 4) as taxa_valido,
    round(100 * totais.total_valido::numeric / nullif(totais.total_fonte, 0), 2) as quality_score_documental
from agregado
inner join totais
    on agregado.fonte_documento = totais.fonte_documento
