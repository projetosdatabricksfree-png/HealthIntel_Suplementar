{{
    config(
        tags=['delta_ans_100', 'tiss_subfamilias']
    )
}}

with base as (
    select
        cast(
            nullif(regexp_replace(cast(competencia as text), '[^0-9]', '', 'g'), '')
            as integer
        )                                               as competencia,
        lpad(trim(coalesce(registro_ans, '')), 6, '0')  as registro_ans,
        lpad(regexp_replace(cast(coalesce(cd_municipio, '') as text), '[^0-9]', '', 'g'), 7, '0')
                                                        as cd_municipio,
        upper(trim(coalesce(nm_municipio, '')))         as nm_municipio,
        upper(trim(coalesce(sg_uf, '')))                as sg_uf,
        upper(trim(coalesce(tipo_evento, '')))          as tipo_evento,
        coalesce(cast(nullif(trim(cast(qt_internacoes as text)), '') as integer), 0)
                                                        as qt_internacoes,
        coalesce(cast(nullif(trim(cast(qt_diarias as text)), '') as integer), 0)
                                                        as qt_diarias,
        cast(nullif(trim(cast(coalesce(vl_pago, '0') as text)), '') as numeric(18, 2))
                                                        as vl_pago,
        cast(nullif(trim(cast(coalesce(vl_informado, '0') as text)), '') as numeric(18, 2))
                                                        as vl_informado,
        _carregado_em,
        _arquivo_origem,
        _lote_id
    from {{ source('bruto_ans', 'tiss_hospitalar') }}
    where trim(coalesce(registro_ans, '')) != ''
)

select *
from base
where competencia is not null
