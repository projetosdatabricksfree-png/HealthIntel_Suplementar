...
with base as (
    select
        ...
        cast(nullif(trim(cast(coalesce(vl_faixa_min, '0') as text)), '') as numeric(18, 2))
                                                        as vl_faixa_min,
        cast(nullif(trim(cast(coalesce(vl_faixa_max, '0') as text)), '') as numeric(18, 2))
                                                        as vl_faixa_max,
        _carregado_em,
        _arquivo_origem,
        _lote_id
    from {{ source('bruto_ans', 'faixa_preco') }}
