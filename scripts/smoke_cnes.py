...
async def _fake_auth(request=None, x_api_key: str | None = None):
    if request is not None:
        request.state.chave_api_id = "cnes_smoke"
        request.state.cliente_id = "cnes_smoke_cliente"
        request.state.plano_id = "cnes_smoke_plano"
        request.state.limite_rpm = 1000
        request.state.endpoint_permitido = ["/v1", "/admin", "/saude", "/prontidao"]
    return x_api_key or "cnes_smoke_key"
        ...
        with TestClient(app) as client:
            resultados = [
                _coletar(
                    client,
                    "GET",
                    "/v1/cnes/municipio/3550308",
                    headers={"X-API-Key": "cnes_smoke_key"},
                ),
                _coletar(
                    client,
                    "GET",
