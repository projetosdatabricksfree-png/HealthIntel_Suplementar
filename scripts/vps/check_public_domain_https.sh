...
FRONTEND_URL="${FRONTEND_URL:-https://app.healthintel.com.br}"
API_BASE_URL="${API_BASE_URL:-https://api.healthintel.com.br}"
ORIGIN="${ORIGIN:-https://app.healthintel.com.br}"
LEGACY_HTTP_API_URL="${LEGACY_HTTP_API_URL:-http://5.189.160.27:8080/saude}"
...
status_code() {
  ...
  shift
  local code
  code="$(curl -sS -o /tmp/healthintel_domain_check_body.txt -w '%{http_code}' "$@" "$url" 2>/dev/null || true)"
...
check_legacy_port_closed() {
  local status
  status="$(status_code "$LEGACY_HTTP_API_URL" --connect-timeout 5)"
  if [ "$status" = "000" ]; then
    ok "Porta 8080 publica fechada"
  else
    fail "Porta 8080 publica ainda respondeu com status $status"
  fi
}

expect_status "Frontend HTTPS" "200" "$FRONTEND_URL" -I
expect_status "API HTTPS /saude" "200" "$API_BASE_URL/saude"
expect_status "API HTTPS /prontidao" "200" "$API_BASE_URL/prontidao"

if [ -n "$ADMIN_KEY" ]; then
  expect_status "API HTTPS /v1/meta/endpoints admin" "200" "$API_BASE_URL/v1/meta/endpoints" -H "X-API-Key: $ADMIN_KEY"
else
  fail "HML_ADMIN_API_KEY ausente para teste de /v1/meta/endpoints"
fi

if [ -n "$DEV_KEY" ]; then
  expect_status "API HTTPS /v1/operadoras dev" "200" "$API_BASE_URL/v1/operadoras?pagina=1&por_pagina=10" -H "X-API-Key: $DEV_KEY"
...
printf '\nResultado: todos os checks por dominio HTTPS passaram.\n'
