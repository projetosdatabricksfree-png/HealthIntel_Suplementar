"""Bootstrap do layout IGR no MongoDB.

GUARD: nao recria layouts ja existentes. Se o layout 'layout_igr_csv' ja
existir com status='ativo', o script falha com instrucoes para usar POST /aliases.

Uso:
    python scripts/bootstrap_layout_registry_igr.py

Pre-requisito: consultar GET /layout/layouts?dataset_codigo=igr antes de rodar.
...
FONTE = FonteDatasetCreate(
    ...
    descricao="Indice de Gestao de Reclamacoes trimestral derivado dos dados ANS.",
...
ALIASES = [
    ...
    LayoutAliasCreate(nome_fisico="Qtd Reclamacoes", destino_raw="total_reclamacoes"),
    LayoutAliasCreate(nome_fisico="Qtd Reclamações", destino_raw="total_reclamacoes"),
...
async def bootstrap() -> None:
    ...
    print(f"[bootstrap_igr] Layout '{layout_id}' criado e ativo.")
