...
- [x] Auditar todos os modelos staging (`stg_*`) em `_staging.yml`: garantir `not_null` em `registro_ans` e `competencia`; `accepted_values` em campos de domínio (modalidade, uf, situacao).
- [x] Auditar todos os intermediate (`int_*`) em `_intermediate.yml`: garantir `not_null` em chaves de join; ausência de duplicatas pós-dedup onde esperado.
- [x] Adicionar testes faltantes em marts existentes (`fat_*`) em `_fato.yml`:
...
- [x] Garantir cobertura completa em `_marts_gold.yml` (Sprint 23) e `_consumo.yml` (Sprint 24) — todos os testes aplicáveis presentes.

### HIS-25.2 — Testes Singulares Adicionais

- [x] `healthintel_dbt/tests/assert_mart_operadora_360_sem_orfaos.sql`:
  - Todo `registro_ans` em `mart_operadora_360` deve existir em `dim_operadora_atual`
- [x] `healthintel_dbt/tests/assert_consumo_score_faixa_valida.sql`:
  - `consumo_score_operadora_mes.score_total between 0 and 100` sem exceções
- [x] `healthintel_dbt/tests/assert_consumo_beneficiarios_nao_negativos.sql`:
  - `consumo_beneficiarios_operadora_mes.qt_beneficiarios >= 0` para todos os registros
- [x] `healthintel_dbt/tests/assert_staging_registro_ans_6_digitos.sql`:
...
- [x] Completar `healthintel_dbt/models/exposures.yml`:
...
- [x] Executar `dbt docs generate` — verificar que todos os modelos têm `description` preenchida (hard gate).
- [x] Salvar artefatos: `docs/dbt_docs_v3.0.0/` com `manifest.json`, `catalog.json`, `index.html`.
...
- [x] Para os seguintes modelos staging com maior consumo, adicionar `columns` completo em `_staging.yml`:
...
- [x] Criar `testes/regressao/test_endpoints_fase4.py` cobrindo:
...
- [x] Atualizar `docs/CHANGELOG.md` com entrada `v3.0.0`:
...
- [x] Atualizar `docs/sprints/README.md` e `docs/sprints/fase4/README.md` (não marcar nenhuma sprint como Concluída antes da aprovação formal).
- [x] Atualizar `CLAUDE.md`: adicionar `consumo_ans` na seção de schemas; `Sprint Structure` com Fase 4; comandos `make consumo-refresh`, `make smoke-prata`, `make smoke-consumo`, `make bootstrap-sib-layouts`, `make bootstrap-cadop-layouts`; role `healthintel_cliente_reader`.
- [x] Criar tag git `v3.0.0` após todos os critérios de saída passarem.

## Entregas esperadas

- [x] `_staging.yml` com column-level docs nos 5 modelos principais
- [x] `_fato.yml`, `_marts_gold.yml`, `_consumo.yml` com testes completos
- [x] Testes singulares `assert_mart_*.sql`, `assert_consumo_*.sql`, `assert_lote_ingestao_sem_duplicata.sql`
- [x] `docs/catalogo_dados/README.md` + 14 páginas de modelos + `glossario.md`
- [x] `docs/arquitetura/lineage_fase4.md`
- [x] `healthintel_dbt/models/exposures.yml` completo
- [x] `docs/dbt_docs_v3.0.0/` com artefatos gerados
- [x] `testes/regressao/test_endpoints_fase4.py`
- [x] `docs/CHANGELOG.md` atualizado com entrada `v3.0.0`
...
- [x] `dbt docs generate` sem `description` vazia nos modelos finais
- [x] `docs/catalogo_dados/` contém README + 14 páginas de modelo + glossário
- [x] Tag `v3.0.0` criada e apontando para commit pós-regressão aprovada
