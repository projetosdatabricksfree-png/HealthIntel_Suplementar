...
- [ ] Criar DDL `infra/postgres/init/022_consumo_ans.sql`:
  ```sql
  ...
  ```
- [ ] Role `healthintel_cliente_reader`: role de grupo `NOLOGIN`, read-only exclusivo em `consumo_ans`; cada cliente deve receber usuário próprio `LOGIN` e `GRANT healthintel_cliente_reader TO <cliente>`.
- [ ] Usuários cliente não acessam `bruto_ans`, `stg_ans`, `int_ans`, `nucleo_ans` ou `plataforma`.
- [ ] Adicionar `consumo_ans` em `healthintel_dbt/dbt_project.yml` como schema gerenciado com macro `generate_schema_name` existente.
- [ ] Criar pasta `healthintel_dbt/models/consumo/` com `_consumo.yml` base.
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_operadora_360.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_beneficiarios_operadora_mes.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_beneficiarios_municipio_mes.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_financeiro_operadora_trimestre.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_regulatorio_operadora_trimestre.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_rede_assistencial_municipio.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_oportunidade_municipio.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/consumo_score_operadora_mes.sql`:
...
- [ ] Criar `healthintel_dbt/models/consumo/_consumo.yml` documentando todos os 8 modelos:
...
- [ ] Adicionar exposures em `healthintel_dbt/models/exposures.yml`:
  ```yaml
...
- [ ] Criar `ingestao/dags/dag_dbt_consumo_refresh.py`:
  - Schedule: `@daily`
  - Tasks:
...
- [ ] Criar `scripts/smoke_consumo.py`:
...
- [ ] Criar `docs/arquitetura/consumo_ans_guia.md` com:
...
- [ ] DDL `infra/postgres/init/022_consumo_ans.sql`
- [ ] Role `healthintel_cliente_reader` com isolamento garantido
- [ ] `models/consumo/consumo_operadora_360.sql`
- [ ] `models/consumo/consumo_beneficiarios_operadora_mes.sql`
- [ ] `models/consumo/consumo_beneficiarios_municipio_mes.sql`
- [ ] `models/consumo/consumo_financeiro_operadora_trimestre.sql`
- [ ] `models/consumo/consumo_regulatorio_operadora_trimestre.sql`
- [ ] `models/consumo/consumo_rede_assistencial_municipio.sql`
- [ ] `models/consumo/consumo_oportunidade_municipio.sql`
- [ ] `models/consumo/consumo_score_operadora_mes.sql`
- [ ] `models/consumo/_consumo.yml` com column-level docs, config e testes
- [ ] `healthintel_dbt/models/exposures.yml` com exposure `consumo_ans_clientes`
- [ ] `ingestao/dags/dag_dbt_consumo_refresh.py`
- [ ] `scripts/smoke_consumo.py`
- [ ] `docs/arquitetura/consumo_ans_guia.md`
- [ ] Makefile com targets `consumo-refresh` e `smoke-consumo`

## Validação esperada

- [ ] `dbt compile --select tag:consumo`
- [ ] `dbt build --select tag:consumo` — sem falhas
- [ ] `dbt test --select tag:consumo` — zero falhas
- [ ] `make consumo-refresh` — sem erros
- [ ] `make smoke-consumo` — zero falhas
- [ ] `make dag-test DAG=dag_dbt_consumo_refresh` — parsing OK
- [ ] Role `healthintel_cliente_reader` acessa `consumo_ans` mas recebe `PERMISSION DENIED` em `bruto_ans`, `stg_ans`, `int_ans`, `nucleo_ans` e `plataforma`
