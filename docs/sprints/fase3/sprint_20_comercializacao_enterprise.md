...
- [ ] Criar migration `infra/postgres/init/020_plano_tiers_fase3.sql` adicionando colunas em `plataforma.plano`:
...
- [ ] Validar que `plataforma.chave_api` tem FK válida para o plano correto de cada cliente demo.

### HIS-20.2 — Rate Limiting por Tier e Camada

- [ ] Atualizar `api/app/middleware/rate_limit.py`:
...
- [ ] Header de resposta `X-RateLimit-Remaining` e `X-RateLimit-Reset` em todas as respostas.
- [ ] Teste unitário `api/tests/unit/test_rate_limit.py` validando multiplicadores por prefixo.

### HIS-20.3 — Billing por Camada

- [ ] Atualizar `plataforma.log_uso`: adicionar coluna `camada TEXT` com valores `'ouro'`, `'prata'`, `'bronze'`.
- [ ] Atualizar `api/app/middleware/log_requisicao.py`: inferir camada pelo prefixo da rota e registrar em `log_uso`.
- [ ] Atualizar script `scripts/billing_close.py`: calcular consumo desagregado por camada para fatura.
- [ ] Criar view `plataforma.vw_consumo_por_camada` agrupando `log_uso` por cliente, camada e mês.

### HIS-20.4 — Catálogo Comercial

- [ ] Criar `docs/comercial/matriz_planos.md` com:
...
- [ ] Criar `docs/comercial/onboarding_enterprise.md`: guia de provisionamento de chave, configuração de plano, teste de integração.
- [ ] Atualizar `GET /v1/meta/endpoints` para incluir campo `camada` (bronze/prata/ouro) e `plano_minimo` por endpoint.

### HIS-20.5 — Suite de Regressão Fase 3

- [ ] Criar `testes/regressao/test_endpoints_fase3.py` cobrindo:
...
- [ ] Executar `pytest testes/regressao/ -v` — zero falhas (hard gate obrigatório para aceite da sprint).

### HIS-20.6 — Baseline v2.0.0

- [ ] Executar `dbt docs generate` e salvar como `docs/dbt_docs_v2.0.0.html` e `docs/dbt_catalog_v2.0.0.json`.
...
- [ ] Criar tag git `v2.0.0-baseline` após todos os critérios de saída passarem.
- [ ] Atualizar `docs/sprints/README.md` e `docs/sprints/fase3/README.md` marcando todas as sprints como Concluída.

## Entregas esperadas

- [ ] DDL `infra/postgres/init/020_plano_tiers_fase3.sql`
- [ ] `api/app/middleware/rate_limit.py` atualizado com multiplicadores por camada
- [ ] `plataforma.log_uso` com coluna `camada`
- [ ] View `plataforma.vw_consumo_por_camada`
- [ ] `docs/comercial/matriz_planos.md`
- [ ] `docs/comercial/onboarding_enterprise.md`
- [ ] `testes/regressao/test_endpoints_fase3.py`
- [ ] `docs/dbt_catalog_v2.0.0.json`
- [ ] `docs/CHANGELOG.md` atualizado
- [ ] Tag git `v2.0.0-baseline`

## Validação esperada

- [ ] `ruff check api ingestao scripts`
- [ ] `pytest testes/regressao/ -v` — zero falhas (hard gate)
- [ ] `pytest api/tests/unit/test_rate_limit.py -v`
- [ ] `dbt docs generate` sem erros
- [ ] `plataforma.plano` contém 5 tiers com `sla_*` e `camadas_permitidas` preenchidos
- [ ] Plano `piloto` não acessa `/v1/bronze/*` nem `/v1/prata/*` (HTTP 403)
- [ ] Plano `enterprise_tecnico` acessa todas as camadas sem rate limit
- [ ] `plataforma.log_uso` registra coluna `camada` para todas as requisições
- [ ] `docs/CHANGELOG.md` contém entrada `v2.0.0` com lista completa de entregáveis
