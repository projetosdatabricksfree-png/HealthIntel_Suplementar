...
- [x] Verificar que `api/app/middleware/autenticacao.py` implementa cache Redis com TTL 60s para hash de chave antes de consultar `plataforma.chave_api` — evitar query PostgreSQL por request (PRD secao 9.2).
- [x] Verificar que `dependencia.py` implementa `verificar_plano()` validando `endpoint_permitido` do plano do cliente em cada request autenticado (PRD secao 9.2).
- [x] Documentar SLO dos endpoints MVP em `docs/operacao/slo_mvp.md`: latencia p95 < 200ms, disponibilidade 99,5%/mes por endpoint (PRD secao 9.3).
- [x] Garantir que CI em `.github/workflows/ci.yml` inclui passo `dbt compile` e validacao de `sqlfluff` nos modelos alterados (PRD secao 14.1 criterios de pronto).
